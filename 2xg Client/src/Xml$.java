...
public class Xml$ implements ActionListener, KeyListener {
	...
	public Xml$(String fileName) {
		...
		try {
			...
			System.out.println((new StringBuilder())
					.append("Couldn't find the file ").append(fileName)
					.toString());
		}
		initUI();
	}
	...
	private void initUI() {
		...
		for (int x = 0; x < itemName.length && itemName[x] != null; x++) {
			if (foundOne) {
				namePanel.append((new StringBuilder()).append(newLine)
						.append(itemName[x]).toString());
				idPanel.append((new StringBuilder()).append(newLine)
						.append(itemID[x]).toString());
			} else {
				foundOne = true;
	...
	private void readStream(InputStream inputStream) throws IOException {
		...
		int linenumber = 0;
		String clientline[];
		for (clientline = new String[30000]; (clientline[linenumber] = reader
				.readLine()) != null; linenumber++) {
		}
		inputStream.close();
		int y = 0;
		for (int x = 0; x < clientline.length && clientline[x] != null; x++) {
			if (clientline[x].contains("name=")) {
				String name = clientline[x].substring(
						clientline[x].indexOf("name=") + 6).replaceAll("\".*",
						"");
				String id = clientline[x].substring(
						clientline[x].indexOf("type=") + 6).replaceAll("\".*",
						"");
				itemName[y] = name;
				itemID[y] = id;
				y++;
