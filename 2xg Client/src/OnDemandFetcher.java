...
public final class OnDemandFetcher extends OnDemandFetcherParent implements
	...
	/*
	 * private boolean crcMatches(int i, int j, byte abyte0[]) { if(abyte0 ==
	 * null || abyte0.length < 2) return false;//false int k = abyte0.length -
	 * 2; int l = ((abyte0[k] & 0xff) << 8) + (abyte0[k + 1] & 0xff);
	 * crc32.reset(); crc32.update(abyte0, 0, k); int i1 = (int)
	 * crc32.getValue(); return l == i && i1 == j; }
	 */
	...
	private void handleFailed() {
		/*
		 * uncompletedCount = 0; completedCount = 0; for(OnDemandData
		 * onDemandData = (OnDemandData) requested.reverseGetFirst();
		 * onDemandData != null; onDemandData = (OnDemandData)
		 * requested.reverseGetNext()) if(onDemandData.incomplete)
		 * uncompletedCount++; else completedCount++;
		 * 
		 * while(uncompletedCount < 10) { OnDemandData onDemandData_1 =
		 * (OnDemandData)aClass19_1368.popHead(); if(onDemandData_1 == null)
		 * break; if(fileStatus[onDemandData_1.dataType][onDemandData_1.ID] !=
		 * 0) filesLoaded++;
		 * fileStatus[onDemandData_1.dataType][onDemandData_1.ID] = 0;
		 * requested.insertHead(onDemandData_1); uncompletedCount++;
		 * closeRequest(onDemandData_1); waiting = true; } CAZ WE DONT LIKE
		 * HANDLE FAILED
		 */
	}
